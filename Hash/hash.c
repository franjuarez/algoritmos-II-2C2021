...
hash_t* hash_crear(hash_destruir_dato_t destruir_dato){
    ...
    if(!hash) return NULL;
    lista_t** arreglo_listas = malloc(sizeof(lista_t*) * CAPACIDAD_INICIAL);
    if(!arreglo_listas){
        free(hash);
        return NULL;
    }
    ...
    for(int i = 0; i < CAPACIDAD_INICIAL;i++) hash->tabla[i] = NULL; //Inicializo todo en NULL
    return hash;
}

bool visitar_redimension(void* dato, void* hash){
    campo_t* campo_actual = dato;
    ...
    hash_aux->cantidad--; //Para dejar la cantidad de igual tamaño
    return true;
}
...
static bool hash_redimensionar(hash_t* hash, size_t capacidad_nueva){
    ...
    for(int j = 0; j < capacidad_nueva; j++) tabla_nueva[j] = NULL;
...
static campo_t* obtener_campo(const hash_t *hash, const char *clave, unsigned long hasheado, bool borrar){
    if(hash->tabla[hasheado] == NULL) return NULL;
    lista_t* lista_actual = hash->tabla[hasheado];
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato){
    unsigned long hasheado = funcion_hash(clave, hash->capacidad);
    if(hash->tabla[hasheado] == NULL){
        lista_t* lista = lista_crear();
        if(!lista) return false;
        hash->tabla[hasheado] = lista;
    }
    if(!hash_pertenece(hash, clave)){
        campo_t* campo = campo_crear(clave, dato);
        if(!campo) return false;
        lista_insertar_ultimo(hash->tabla[hasheado], campo);
    }
    else{
        campo_t* campo_renovar = obtener_campo(hash, clave, hasheado, false);
    ...
    if(calcular_factor_de_carga(hash) > FACTOR_CARGA_MAXIMO){
        hash_redimensionar(hash, (hash->capacidad) * REDIMENSION);
    }
    return true;
}
...
void *hash_borrar(hash_t *hash, const char *clave){ 
    unsigned long hasheado = funcion_hash(clave, hash->capacidad);
    campo_t* campo_buscado = obtener_campo(hash, clave, hasheado, true);
    if(!campo_buscado) return NULL;
    void* dato = campo_buscado->dato;
    campo_destruir(campo_buscado);
    if(lista_esta_vacia(hash->tabla[hasheado])){
        lista_destruir(hash->tabla[hasheado], NULL);
        hash->tabla[hasheado] = NULL;
    }
    hash->cantidad--;
...
void *hash_obtener(const hash_t *hash, const char *clave){
    unsigned long hasheado = funcion_hash(clave, hash->capacidad);
    campo_t* campo_buscado = obtener_campo(hash, clave, hasheado, false);
    if(!campo_buscado) return NULL;
...
static void buscar_lista(hash_iter_t* iter){
    ...
    for(; i < iter->hash->capacidad; i++) if(iter->hash->tabla[i] != NULL) break;
